...
io.on("connection", (socket) => {
  ...
  socket.on("placeMonster", (data) => {
    ...
    if (!game || !game.currentPlayer || game.currentPlayer.id !== playerId) {
      console.log("Invalid placeMonster attempt:", {
        playerId,
  ...
  socket.on("moveMonster", (data) => {
    ...
    if (!game || !game.currentPlayer || game.currentPlayer.id !== playerId) {
      console.log("Invalid moveMonster attempt:", {
        playerId,
        currentPlayer: game?.currentPlayer?.id,
      });
      return;
  ...
  socket.on("endTurn", (data) => {
    ...
    if (!game || !game.currentPlayer || game.currentPlayer.id !== playerId) {
      console.log("Invalid endTurn attempt:", {
        playerId,
        currentPlayer: game?.currentPlayer?.id,
      });
      return;
...
setInterval(() => {
  Object.entries(games).forEach(([room, game]) => {
    console.log(
      `Room ${room} current player:`,
      game.currentPlayer?.id || "none"
    );
  });
}, 5000);
